...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition=='S':
            ...
            self.dependencies.append((self.stack[stlen-2],self.stack[stlen-1]))
            self.stack.pop(stlen-1)
        else:
            stlen=len(self.stack)
            self.dependencies.append((self.stack[stlen-1],self.stack[stlen-2]))
            self.stack.pop(stlen-2)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)!=0:
        ...
        predictions=model.predict(minibatches)
        for predict,partial_parse in zip(predictions,minibatches):
            partial_parse.parse_step(predict)
        unfinished_parses=[parse for parse in unfinished_parses if len(parse.stack)!=1 or len(parse.buffer)!=0]
    dependencies=[dependency.dependencies  for dependency in partial_parses]
