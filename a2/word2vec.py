...
def naiveSoftmaxLossAndGradient(
    ...
    ### END YOUR CODE
    arr_exp=np.exp(np.matmul(outsideVectors,centerWordVec))
    arr_exp_sum=np.sum(arr_exp)
    pred=arr_exp/arr_exp_sum

    label=np.zeros(outsideVectors.shape[0])
    label[outsideWordIdx]=1

    loss=-np.log(pred[outsideWordIdx])
    gradCenterVec=np.dot(np.transpose(outsideVectors),pred-label)
    gradOutsideVecs=np.reshape(pred-label,(outsideVectors.shape[0],1))*centerWordVec
...
def negSamplingLossAndGradient(
    ...
    sample_vector=outsideVectors[indices]
    mat=np.matmul(sample_vector,centerWordVec)
    label=np.ones(K+1)*(-1)
    label[0]=1
    pred=sigmoid(label*mat)

    loss=np.sum(-np.log(pred))
    gradCenterVec=np.matmul(np.transpose(sample_vector),(pred-1)*label)

    gradOutsideVecs=np.zeros(outsideVectors.shape)
    grads=np.reshape((pred-1)*label,(K+1,1))*centerWordVec
    for idx,grad in zip(indices,grads):
        gradOutsideVecs[idx]+=grad
    ### END YOUR CODE
    #assert(np.sum(test)==np.sum(gradOutsideVecs))
...
# ):
    ...
    for outsideWord in outsideWords:
        ...
        # 计算损失和梯度
        currLoss, currGradCenterVecs, currGradOutsideVecs = word2vecLossAndGradient(centerWordVectors[centerWordIdx], outsideWordIdx, outsideVectors, dataset)
        # 累加损失和梯度
        loss += currLoss
        gradCenterVecs[centerWordIdx] += currGradCenterVecs
