...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn=nn.Conv1d(embed_size, embed_size, 2, 1,'same')
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        x=x.permute(2,0,1)
        x=nn.utils.rnn.pack_padded_sequence(x,source_lengths,batch_first=False)
        enc_hiddens,(last_hidden,last_cell)=self.encoder(x)
        enc_hiddens,source_lengths=nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens=enc_hiddens.permute(1,0,2)
        init_decoder_hidden=self.h_projection(last_hidden.permute(1,0,2).reshape((last_hidden.shape[1],2*last_hidden.shape[2])))
        init_decoder_cell=self.c_projection(last_cell.permute(1,0,2).reshape((last_cell.shape[1],2*last_cell.shape[2])))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_t=torch.unsqueeze(dec_hidden,1)
        dec_hidden_t=dec_hidden_t.permute(0,2,1)
        e_t=torch.squeeze(torch.matmul(enc_hiddens_proj,dec_hidden_t),dim=2)
        ...
        a_t=torch.unsqueeze(alpha_t,1)@enc_hiddens
